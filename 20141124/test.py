...
def readmatrix(readmatrix_trace_noun="./名词集合.txt",readmatrix_trace_topic="./inference.txt"):
    """读取名词以及名词的语义分布存为数组"""
...
def KL_dis(KL_distribution1,KL_distribution2):
    ...
    for i in range(len(KL_distribution1)):
        ...
        C_entropy2+=temp2
    return -(C_entropy2+C_entropy1)/2
...
for k1 in matrix:
    for k2 in matrix:
        if k1!=k2 and set([k1,k2]) not in component:
            similarity=cosine(matrix[k1],matrix[k2])
            similar_dict[tuple([k1,k2])]=similarity
            component.append(set([k1,k2]))
        else:
            pass
h=open("./名词的余弦相似性.txt",'w')
