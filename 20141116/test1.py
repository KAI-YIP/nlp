...
class TextProcess:
    ...
    def wordchoose(wordchoose_array):
        """筛选名词"""
        new_txt=[]
        for word in wordchoose_array:
            if word[0]=="\n":
                new_txt.append(word)
            else:
                if "n" in word[1]:
    ...
    def array2str(array2str_array):
        ...
        for word in array2str_array:
            if word[0]!="\n":
                temp_list.append(str(word[0])+"/"+str(word[1]))
            else:
...
if __name__=='__main__':
    for i in range(10):
        i+=1
        txt1=TextProcess.wordseg("./data/product"+str(i)+".txt")
        txt3=TextProcess.santencebreak(txt1)
    #    txt4=TextProcess.wordchoose(txt3)
        txt5=TextProcess.stopwordRM(txt3)
        TextProcess.savearray(txt5,"./data/product1temp.txt")
        #txt2=TextProcess.array2str(txt5)
        #TextProcess.savefile(txt2,"./data/product"+str(i)+"sentence.txt")
        print ("product"+str(i)+"result is finished!")
        break